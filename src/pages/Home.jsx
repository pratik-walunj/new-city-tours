  ...
  return (
    <motion.div variants={pageVariants} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.4 }}>
      ...
      <section className="py-16 bg-slate-900 relative overflow-hidden">
        <div
          className="absolute inset-0 opacity-20"
          style={{ backgroundImage: 'url(https://images.unsplash.com/photo-1549317661-bd32c8ce0db2?w=1200&q=50)', backgroundSize: 'cover', backgroundPosition: 'center' }}
        />
        <div className="absolute inset-0 bg-slate-900/80" />
        <div className="relative max-w-7xl mx-auto px-4">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8">
            <Reveal>
              <p className="text-orange-400 text-xs font-bold uppercase tracking-widest mb-2">New Service</p>
              <h2 className="text-3xl md:text-4xl font-extrabold text-white leading-tight">
                Nepal Taxi & Cab Services
              </h2>
              <p className="mt-3 text-cyan-100/80 max-w-xl">
                Airport transfers, city rides, Muktinath jeeps and luxury chauffeur SUVs — GPS-tracked, fixed fares, 24x7 dispatch. Six vehicle types for every need.
              </p>
              <div className="mt-5 flex flex-wrap gap-3">
                {['Sedan from ₹350', 'SUV from ₹500', 'Luxury Crysta from ₹900', 'Group Van from ₹800'].map((t) => (
                  <span key={t} className="bg-white/10 border border-white/20 text-white text-xs font-semibold px-3 py-1.5 rounded-full">{t}</span>
                ))}
              </div>
            </Reveal>
            <Reveal delay={150} className="shrink-0">
              <Link
                to="/taxi-cab"
                className="inline-flex items-center gap-2 px-8 py-4 rounded-2xl bg-orange-500 hover:bg-orange-600 text-white font-bold shadow-2xl shadow-orange-500/30 hover:scale-105 transition-all text-lg"
              >
                View All Cabs <ArrowRight size={20} />
              </Link>
            </Reveal>
          </div>
        </div>
